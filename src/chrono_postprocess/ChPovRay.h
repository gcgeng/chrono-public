...
class ChApiPostProcess ChPovRay : public ChPostProcessBase {
  public:
    ...
    void ExportScript(int simulation_frame_cnt) { ExportScript(out_script_filename, simulation_frame_cnt); };
    void ExportScript(const std::string& filename, int simulation_frame_cnt);

    /// As ExportScript(), but overrides the filename.
    virtual void ExportScript(const std::string& filename) override {
      ExportScript(filename, 999);
    }
